...
# === ВНУТРИ СИСТЕМЫ ===
    ...
    if st.session_state.sos_mode:
            ...
            if delta == 0 and st.session_state.streak > 0:
                ...
                if st.button("✨ СЕГОДНЯ ЧИСТ", use_container_width=True):
                    ...
                    if 'frequency' not in profile:
                        st.session_state.calibration_step = 1 # СТАРТ С ВОПРОСА 1 (КНИГА)
                        first_msg = "День зачтен. Но прежде чем мы настроим защиту, ответь:\n\n1. **Ты уже читал книгу 'Кто такой Алкоголь'?** (Да/Нет)"
                    else:
        ...
        if prompt := st.chat_input("Ввод данных..."):
            ...
            # === ЛОГИКА КАЛИБРОВКИ (ОБНОВЛЕННАЯ ПОСЛЕДОВАТЕЛЬНОСТЬ) ===
            step = st.session_state.calibration_step
            if step > 0:
                next_msg = ""
                if step == 1:
                    update_onboarding_data(st.session_state.row_num, "read_book", prompt)
                    next_msg = "Принято. Вопрос 2.\n**Как часто Паразит обычно атакует?** (Каждый день, по пятницам, запоями?)"
                ...
                if not st.session_state.vip and sum(1 for m in st.session_state.messages if m["role"] == "user") >= limit:
                    ...
                    with st.chat_message("assistant"):
                        with st.spinner("PROCESSING..."):
                            ...
                            - Читал книгу: {profile.get('read_book', 'Неизвестно')}
